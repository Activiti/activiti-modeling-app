...
import { SnackbarErrorAction, EntityDialogForm } from '../../../store/actions';
...
import { SnackbarInfoAction } from '../../../store/actions/snackbar';
import { selectSelectedAppId } from '../../../store/selectors/app.selectors';
...
export class ProcessesEffects extends BaseEffects {
    ...
    createProcessEffect = this.actions$.pipe(
        ...
        mergeMap(([form, appId]) => this.createProcess({ ...form, name: form.name + PROCESS_FILE_FORMAT }, appId))
    );
    ...
    private getProcesses(applicationId: string): Observable<{} | GetProcessesSuccessAction> {
        return this.applicationEditorService.fetchProcesses(applicationId).pipe(
            ...
            catchError(e =>
                this.genericErrorHandler(this.handleError.bind(this, 'APP.APPLICATION.ERROR.LOAD_VIRTUAL_NODES'), e)
            )
        );
    }
