...
const propertyName = BpmnProperty.type;

export function createTimerTypeProperty({ element, appConfigService }: FactoryProps) {
    return new CardViewSelectItemModel({
        label: 'PROCESS_EDITOR.ELEMENT_PROPERTIES.TIMER_TYPE',
        options$: of(appConfigService.get('process-modeler.timer-types')),
