...
import { JSONSchemaInfoBasics, HxPDocumentType, MinimalModelSummary, HXP_DOC_TYPE } from '../../../api/types';
import { ModelApiVariation } from '../model-api';
import { ContentType } from '../content-types';
import { HXP_DOC_TYPE_FILE_FORMAT } from '../../../helpers/utils/create-entries-names';
import { ModelContentSerializer } from '../model-content-serializer';
import { ModelDataExtractor } from '../model-data-extractor';
import { extractHxpDocumentTypeData } from './model-data-extractors/hxp-document-type-data-extractor';

@Injectable()
export class HxPDocumentTypeApiVariation<M extends HxPDocumentType, C extends JSONSchemaInfoBasics> implements ModelApiVariation<M, C> {
    readonly contentType = ContentType.HxPDocumentType;
    readonly retrieveModelAfterUpdate = false;

    constructor(
        private serializer: ModelContentSerializer<JSONSchemaInfoBasics>,
        private dataExtractor: ModelDataExtractor<JSONSchemaInfoBasics, HxPDocumentType>
    ) {
        ...
        this.dataExtractor.register({ type: HXP_DOC_TYPE, get: extractHxpDocumentTypeData });
    }
    ...
    public getModelFileName(model: Partial<M>): string {
        return model.name + HXP_DOC_TYPE_FILE_FORMAT;
    }
