...
const propertyKey = BpmnProperty.candidateStarterUsers;

const get = element => {
    let candidateStarterUsers;
    if (element.type === BpmnElement.Process) {
        candidateStarterUsers = element.businessObject[propertyKey];
    } else if (element.type === BpmnElement.Participant) {
        candidateStarterUsers = element.businessObject.processRef[propertyKey];
    }
...
export const candidateStarterUsersHandler = { get, set };
